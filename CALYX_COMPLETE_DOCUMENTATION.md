...
> **Current-state patch (2026-07-07, repo HEAD `8310d211`).** This copy was updated in place against `C:\code\Calyx-Dev` on `main` at `8310d211` (`Make Poly runbooks public-mirrorable`). It now includes the committed post-`57fcd478` state: `calyx-poly` as the 24th workspace crate, Poly public runbooks, the biomedical blindspot audit CLI and FSV documentation, the workspace `tungstenite`/`ureq` dependency inheritance fix, and refreshed file/command counts. The source tree was clean when this patch was applied.
...
- **`calyx-poly` imported as a workspace crate** (`db5ebab3`, finished through `8310d211`) —
  the workspace now has **24 crates**. `calyx-poly` is a local/read-only Polymarket
...
`8310d211` the workspace holds **24 crates**: the 23-crate `57fcd478` state plus
`calyx-poly`. Pinned invariants rechecked for this document: MCP tools **31**
...
holds **24 crates** — `calyx-poly` was added after `57fcd478`; `calyx-buildinfo`
...
>
> **Current-count correction (8310d211):** the workspace now has **24 crates** and **1,803**
> tracked `.rs` files under `crates/*/src/`. The extra crate is `calyx-poly`; current
