...
use calyx_aster::ledger_view::{AsterLedgerCfStore, read_ledger_seqs_traced};
...
pub(crate) fn attach_verified_provenance(
    ...
    let mut ledger = TargetedLedgerVerifier::open(vault_dir, ledger_view, hits, docs, trace)?;
...
impl TargetedLedgerVerifier {
    fn open(
        ...
        let rows = if let Some(ledger_view) = ledger_view {
            let mut rows = BTreeMap::new();
            for seq in &required {
                if let Some(row) = ledger_view.read_seq(*seq)? {
                    rows.insert(*seq, row);
                }
            }
            rows
        ...
        trace.emit_detail(
            "provenance.ledger_snapshot_read.done",
            None,
            Some(rows.len()),
            Some(format!(
                "view={} wanted={} resolved={} physical_reopens={}",
                if ledger_view.is_some() {
                    "aster_ledger_cf_store_snapshot"
                } else {
                    "targeted_ledger_point_read"
                },
                required.len(),
                rows.len(),
                usize::from(ledger_view.is_none())
            )),
        );
        Ok(Self {
            rows,
