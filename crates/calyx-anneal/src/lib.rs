...
pub use learn::{
    ...
    HeadKind, HeadPromotionGate, HeadReadback, HeadRegressionRollback, HeadStorage,
    HeadUpdateOutcome, HeadUpdateSummary, MAX_ONLINE_HEAD_PARAMS, MistakeEntry, MistakeLog,
    MistakeReadback, MistakeRef, MistakeStorage, NoFrozenLensGuard, OnlineHead, OnlineHeadState,
    ...
    RecordOutcomeReward, RegressionConfig, RegressionContextSource, RegressionPredictor,
    RegressionReport, RegressionResult, RegressionUpdateOutcome, ReplayBuffer, ReplayEntry,
    ReplaySnapshot, ReplayStorage, SleepPassConfig, SleepPassOutcome, SleepPassReplayRecord,
    assert_no_regression, decode_head_rows, decode_mistake_entry, decode_online_head,
    decode_outcome_queue_entry, decode_replay_snapshot, encode_mistake_entry, encode_online_head,
    encode_outcome_queue_entry, encode_replay_snapshot, head_key, head_state_artifact_key,
    mistake_key, mistake_seq_from_key, outcome_queue_key, outcome_queue_seq_from_key,
    record_mistake_for_replay, record_outcome, record_regression, regression_rate,
    regression_recurred, replay_snapshot_key, run_sleep_pass,
};
...
pub use tune::{
    ...
    CALYX_ANNEAL_SOAK_TIME_BUDGET_EXHAUSTED, CALYX_ANNEAL_TRIAL_ALREADY_ACTIVE,
    CALYX_ANNEAL_TRIAL_INVALID_RESULT, CALYX_ANNEAL_TRIAL_NOT_ACTIVE, CALYX_FORGE_CACHE_WRITE_FAIL,
    CALYX_FORGE_SCOPE_INVALID_CONFIG, CALYX_INDEX_CACHE_WRITE_FAIL,
    CALYX_INDEX_SCOPE_INVALID_CONFIG, CALYX_LOOM_PLAN_WRITE_FAIL, CALYX_LOOM_SCOPE_INVALID_CONFIG,
    CALYX_STORAGE_CACHE_WRITE_FAIL, CALYX_STORAGE_SCOPE_INVALID_CONFIG, ConcatKey, ConfigBandit,
    ConfigBanditStore, ConfigVariant, DEFAULT_AB_MIN_SAMPLES, DEFAULT_FORGE_RECALL_TARGET,
    DEFAULT_HYSTERESIS_WINS, DEFAULT_INDEX_RECALL_TARGET, DEFAULT_INDEX_VRAM_BUDGET_BYTES,
    DEFAULT_LOOM_RECALL_TARGET, DEFAULT_SOAK_OSCILLATION_WINDOW, DEFAULT_SOAK_P99_TARGET_REDUCTION,
    DEFAULT_SOAK_QUERIES, DEFAULT_SOAK_SAMPLE_INTERVAL, DEFAULT_SOAK_SEED,
    DEFAULT_STORAGE_RECALL_TARGET, DType, ForgeBanditPersistence, ForgeConfig,
    ...
    IndexScopeTuner, IndexSlotHealth, IndexTuneDecision, IndexTuneSkip, LoomBanditPersistence,
    LoomMaterializer, LoomPromotionRecord, LoomPromotionWriter, LoomScopeTuner, LoomTuneDecision,
    MAX_BUCKETED_DIM, MAX_FORGE_CANDIDATES, MAX_INDEX_CANDIDATES, MAX_LOOM_CANDIDATES,
    MAX_LOOM_EAGER_PAIRS, MAX_STORAGE_CANDIDATES, MIN_BITS_PER_ANCHOR, MIN_LOOM_PAIR_BITS,
    MatPlanConfig, MetricSample, NoopABBudget, NoopABLedgerWriter, NoopForgeBanditStore,
