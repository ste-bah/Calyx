...
fn candle_contract(
    ...
    // (Upstream extracted our f16/bf16->f32 finite-replay rule into
    // contract_finite_replay_precision; the corpus-hash inputs are unchanged,
    // so frozen LensIds stay stable — verified by the resident loading all 7
    // GPU content lenses on the existing tolkien-full vault after deploy.)
    let finite_replay =
