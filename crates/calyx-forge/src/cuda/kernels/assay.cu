...
extern "C" __global__ __launch_bounds__(ASSAY_THREADS) void assay_histogram_pair_f32(
    const float *x,
    const float *y,
    int n,
    int bins,
    ...
    float y_min,
    float y_width,
    unsigned long long *x_counts,
    unsigned long long *y_counts,
    unsigned long long *joint_counts,
    unsigned int *flags) {
    const int i = (int)(blockIdx.x * blockDim.x + threadIdx.x);
    if (i >= n) {
        return;
    }
    if (n <= 0 || bins < 2 || !isfinite(x_width) || !isfinite(y_width) ||
        x_width <= 0.0f || y_width <= 0.0f) {
        atomicOr(flags, ASSAY_FLAG_INVALID_INDEX);
        return;
    }
    const float xv = x[i];
    ...
    int xb = (int)floorf(((xv - x_min) / x_width) * (float)bins);
    int yb = (int)floorf(((yv - y_min) / y_width) * (float)bins);
    xb = max(0, min(xb, bins - 1));
    yb = max(0, min(yb, bins - 1));
    atomicAdd(&x_counts[xb], 1ULL);
    atomicAdd(&y_counts[yb], 1ULL);
    atomicAdd(&joint_counts[xb * bins + yb], 1ULL);
}

extern "C" __global__ __launch_bounds__(ASSAY_THREADS) void assay_contingency_u32(
    ...
    const unsigned int row = x[i];
    const unsigned int col = y[i];
    if (n <= 0 || rows < 2 || cols < 2 || row >= (unsigned int)rows ||
        col >= (unsigned int)cols) {
        atomicOr(flags, ASSAY_FLAG_INVALID_INDEX);
        return;
    }
    atomicAdd(&table[(size_t)row * (size_t)cols + col], 1ULL);
}

extern "C" __global__ __launch_bounds__(ASSAY_THREADS) void assay_rank_ties_f64(
    const double *x,
    const double *y,
    int n,
    double *x_ranks,
    double *y_ranks,
    unsigned int *x_tie_sizes,
    unsigned int *y_tie_sizes,
    unsigned int *flags) {
    const int i = (int)(blockIdx.x * blockDim.x + threadIdx.x);
    if (i >= n) {
        return;
    ...
    const double xv = x[i];
    const double yv = y[i];
    if (n <= 0 || !isfinite(xv) || !isfinite(yv)) {
        atomicOr(flags, n <= 0 ? ASSAY_FLAG_INVALID_INDEX : ASSAY_FLAG_NONFINITE);
        return;
    }
    unsigned int x_less = 0u;
    unsigned int y_less = 0u;
    unsigned int x_equal = 0u;
    unsigned int y_equal = 0u;
    bool x_leader = true;
    bool y_leader = true;
    for (int j = 0; j < n; ++j) {
        const double xj = x[j];
        ...
        x_equal += xj == xv;
        y_equal += yj == yv;
        x_leader = x_leader && !(j < i && xj == xv);
        y_leader = y_leader && !(j < i && yj == yv);
    }
    x_ranks[i] = (double)x_less + ((double)x_equal + 1.0) * 0.5;
    y_ranks[i] = (double)y_less + ((double)y_equal + 1.0) * 0.5;
    x_tie_sizes[i] = x_leader && x_equal > 1u ? x_equal : 0u;
    y_tie_sizes[i] = y_leader && y_equal > 1u ? y_equal : 0u;
}

extern "C" __global__ __launch_bounds__(ASSAY_THREADS) void assay_kendall_counts_f64(
    const double *x,
    const double *y,
    int n,
    ...
    unsigned long long local_concordant = 0ULL;
    unsigned long long local_discordant = 0ULL;
    const double xv = x[i];
    const double yv = y[i];
    if (!isfinite(xv) || !isfinite(yv)) {
        atomicOr(flags, ASSAY_FLAG_NONFINITE);
    ...
    for (int j = i + 1; j < n; ++j) {
        const double dx = xv - x[j];
        const double dy = yv - y[j];
        if (!isfinite(dx) || !isfinite(dy)) {
            atomicOr(flags, ASSAY_FLAG_NONFINITE);
            return;
        }
        const double product = dx * dy;
        local_concordant += product > 0.0;
        local_discordant += product < 0.0;
    }
    atomicAdd(concordant, local_concordant);
    atomicAdd(discordant, local_discordant);
}

extern "C" __global__ __launch_bounds__(ASSAY_THREADS) void assay_copula_terms_f64(
    const double *x_ranks,
    const double *y_ranks,
    int n,
    double tail_q,
    unsigned long long *c_mid_count,
    unsigned long long *lower_tail_count,
    unsigned long long *upper_tail_count,
    double *hoeffding_terms,
    double *gini_terms,
    unsigned int *flags) {
    const int i = (int)(blockIdx.x * blockDim.x + threadIdx.x);
    if (i >= n) {
    ...
    if (n <= 0 || !isfinite(tail_q) || tail_q <= 0.0 || tail_q >= 0.5) {
        atomicOr(flags, ASSAY_FLAG_INVALID_INDEX);
        return;
    }
    const double scale = 1.0 / (double)(n + 1);
    const double u = x_ranks[i] * scale;
    const double v = y_ranks[i] * scale;
    unsigned long long at_uv = 0ULL;
    const double t = (double)(i + 1) * scale;
    unsigned long long at_diag = 0ULL;
    unsigned long long at_anti = 0ULL;
    for (int j = 0; j < n; ++j) {
        const double uj = x_ranks[j] * scale;
        const double vj = y_ranks[j] * scale;
        at_uv += uj <= u && vj <= v;
        at_diag += uj <= t && vj <= t;
        at_anti += uj <= t && vj <= 1.0 - t;
    }
    const double c = (double)at_uv / (double)n;
    const double delta = c - u * v;
    hoeffding_terms[i] = delta * delta;
    gini_terms[i] = ((double)at_diag + (double)at_anti) / (double)n;
    if (!(isfinite(hoeffding_terms[i]) && isfinite(gini_terms[i]))) {
        atomicOr(flags, ASSAY_FLAG_NONFINITE);
        return;
    }
    atomicAdd(c_mid_count, (unsigned long long)(u <= 0.5 && v <= 0.5));
    atomicAdd(lower_tail_count, (unsigned long long)(u <= tail_q && v <= tail_q));
    atomicAdd(upper_tail_count,
              (unsigned long long)(u >= 1.0 - tail_q && v >= 1.0 - tail_q));
}

__device__ __forceinline__ double assay_mic_wh(
    const unsigned int *cum,
    unsigned long long offset,
    const double *weighted_log2,
    int secondary_bins,
    int i,
    int m) {
    const int total = m - i;
    double value = weighted_log2[total];
    for (int r = 0; r < secondary_bins; ++r) {
        const unsigned int count =
            cum[offset + (unsigned long long)m * secondary_bins + r] -
            cum[offset + (unsigned long long)i * secondary_bins + r];
        if (count > 0u) {
            value -= weighted_log2[count];
        }
    }
    return value;
}

extern "C" __global__ void assay_mic_candidates_f64(
    const unsigned int *cumulative_counts,
    const unsigned long long *cumulative_offsets,
    const unsigned int *cut_allowed,
    const unsigned long long *cut_offsets,
    const int *secondary_bins,
    const int *max_primary_bins,
    const double *weighted_log2,
    const double *entropy_terms,
    const double *integer_log2,
    int n,
    int candidate_count,
    double *scratch,
    double *scores,
    int *primary_bins_out,
    int *secondary_bins_out,
    unsigned int *flags) {
    const int candidate = (int)blockIdx.x;
    if (candidate >= candidate_count || threadIdx.x != 0) {
        return;
    }
    const int s = secondary_bins[candidate];
    const int max_primary = max_primary_bins[candidate];
    const unsigned long long cum_offset = cumulative_offsets[candidate];
    const unsigned long long cut_offset = cut_offsets[candidate];
    if (n <= 0 || s < 2 || max_primary < 2) {
        atomicOr(flags, ASSAY_FLAG_INVALID_INDEX);
        return;
    }
    double *prev = scratch + (unsigned long long)candidate * 2ULL * (n + 1);
    double *curr = prev + (n + 1);
    prev[0] = INFINITY;
    for (int m = 1; m <= n; ++m) {
        prev[m] = assay_mic_wh(cumulative_counts, cum_offset, weighted_log2, s, 0, m);
    }
    const double nf = (double)n;
    double h_secondary = 0.0;
    for (int r = 0; r < s; ++r) {
        const unsigned int count =
            cumulative_counts[cum_offset + (unsigned long long)n * s + r];
        if (count > 0u) {
            h_secondary -= entropy_terms[count];
        }
    }
    double best = 0.0;
    int best_primary = 2;
    for (int l = 2; l <= max_primary; ++l) {
        for (int m = 0; m < l; ++m) {
            curr[m] = INFINITY;
        }
        for (int m = l; m <= n; ++m) {
            ...
            for (int i = l - 1; i < m; ++i) {
                if (cut_allowed[cut_offset + i] == 0u || !isfinite(prev[i])) {
                    continue;
                }
                const double candidate_cost = prev[i] + assay_mic_wh(
                    cumulative_counts, cum_offset, weighted_log2, s, i, m);
        ...
        if (isfinite(curr[n])) {
            const double mi = fmax(h_secondary - curr[n] / nf, 0.0);
            const double denominator = integer_log2[min(l, s)];
            const double score = denominator > 0.0 ? mi / denominator : 0.0;
            if (!isfinite(score)) {
                atomicOr(flags, ASSAY_FLAG_NONFINITE);
                return;
            }
    ...
    primary_bins_out[candidate] = best_primary;
    secondary_bins_out[candidate] = s;
}
