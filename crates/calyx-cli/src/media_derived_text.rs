...
pub(crate) fn derive_text_for_media(
    vault_dir: &Path,
    retained: &RetainedMediaInput,
    source_cx_id: CxId,
) -> CliResult<DerivedTextArtifact> {
    ...
    eprintln!(
        "CALYX_MEDIA_DERIVED_TEXT phase=run_start cmd={} modality={} kind={} input={} output={}",
    ...
    eprintln!(
        "CALYX_MEDIA_DERIVED_TEXT phase=run_ok source_cx_id={} kind={} text_sha256={} text_bytes={}",
        source_cx_id,
        kind,
        text_sha256,
        text_bytes.len()
...
pub(crate) fn derivation_ledger_payload(
    retained: &RetainedMediaInput,
...
fn derived_command_output_path(vault_dir: &Path) -> CliResult<PathBuf> {
...
fn read_derived_output(path: &Path) -> CliResult<DerivedTextOutput> {
...
fn write_derived_text_blob(path: &Path, bytes: &[u8]) -> CliResult {
